...
function priceScore(model) {
  if (model.lowestKnownPrice === null) return 0;
  return 1 / (1 + model.lowestKnownPrice);
}
...
export function selectModel(models, { modality, mode = 'balanced', requestedModel } = {}) {
  ...
  if (mode === 'economy') {
    return [...compatible].sort((a, b) => {
      const aPrice = a.lowestKnownPrice ?? Number.POSITIVE_INFINITY;
      const bPrice = b.lowestKnownPrice ?? Number.POSITIVE_INFINITY;
